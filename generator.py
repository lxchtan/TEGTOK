...
def main():
  ...
  # dataset_class = getattr(dataloader, "testDataset" if not args.debug else "testDataset_debug") # TODO: suitable
  ...
  for did, batch in enumerate(tqdm(test_loader, desc="Generating", disable=args.debug)):
    with torch.no_grad():
      sampled_output_ids, ground_truth, dialog_id, history, *tg_ids = run_batch_generation_sample(batch, args, model, test_dataset)
      sampled_output_text = tokenizer.batch_decode(sampled_output_ids, skip_special_tokens=True)
      
      tg_text = [""] * len(sampled_output_text)
      if len(tg_ids) > 0:
        tg_text = [tg[1:] for tg in tokenizer.batch_decode(tg_ids[0], skip_special_tokens=True)]
      knowledge_indexs = model.knowledge_index.cpu().numpy().tolist() if getattr(model, 'knowledge_index', None) is not None else [0] * len(ground_truth)

      for didt, gt, res, his, tg, ki in zip(dialog_id, ground_truth, sampled_output_text, history, tg_text, knowledge_indexs):
        example = {}
        example["dialog_id"] = didt
        example["ground_truth"] = gt
        example["generated"] = res
        example["reddit_knowledge"] = tg
        example["reddit_index"] = ki
